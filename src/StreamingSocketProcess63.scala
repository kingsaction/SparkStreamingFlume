...
object StreamingSocketProcess63 extends Logging {
...
//    val socketPort = PROPERTIES.getProperty("socket.port").toInt
    ...
    val sparkConf = new SparkConf().setMaster(masterUrl).setAppName("StreamingSocketProcess64").set("spark.driver.allowMultipleContexts", "true")
...
//    sparkConf.set("spark.streaming.unpersist","true")
    ...
    val s64 =  ssc.socketTextStream(socketHost, socketPort.toInt, StorageLevel.MEMORY_AND_DISK_SER_2).repartition(1)
    s64.persist(StorageLevel.MEMORY_AND_DISK_SER_2)
    s64.foreachRDD(rdd => {
    ...
    //源数据存储HDFS
    s64.saveAsTextFiles("hdfs://sparkCluster/xinling/socket/source1/s64/"+dt1+"/")
